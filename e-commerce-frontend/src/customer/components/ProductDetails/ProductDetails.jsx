...
const fetchsimilarProducts = async()=>{
  try {
    const responseProducts = await api.get(`/api/products?category=${products?.product?.category?.name}`);
    // console.log("same category coming ?",products?.product?.category?.name)
    if (responseProducts && responseProducts.data) {
      setSame(responseProducts.data);
      // console.log('similarproductsdata:', responseProducts.data);
    }
  } catch (error) {
...
const [isLoading, setIsLoading] = useState(false);
  ...
  const handleAddToCart=()=>{
    ...
    console.log("data _ cart wala",data) 
    dispatch(addItemToCart(data))
    
  
    setIsLoading(false);
  }
  console.log("cartdebug ",cart)  
  const handleButtonClickxx = () => {
    if (auth.user) {
        ...
        lg:pb-24">
          ...
          <div className="mt-4 lg:row-span-3 lg:mt-0">
            ...
            <form  className="mt-10">
              ...
              {/* <Button onClick={handleButtonClickxx} disabled={!selectedSize || isLoading} variant="contained" sx={{px:"2rem", py:"1rem", bgcolor:"#9155fd"}}>Add To Cart</Button>
              { showPopup && <Popup isOpen={showPopup} onClose={()=>setShowPopup(false)} />} */}
             <Tooltip title={!selectedSize ? "Please select size" : ""} arrow>
        <span>
          <Button
            onClick={handleButtonClickxx}
            disabled={!selectedSize || isLoading} // Disabled if no size is selected or isLoading is true
            variant="contained"
            sx={{ px: "2rem", py: "1rem", bgcolor: "#9155fd" ,'&.disabled': {  border: '2px solid red' , '&:hover': {borderColor: 'red',},    },}}
            className={!selectedSize || isLoading ? 'disabled' : ''}
                >
            Add To Cart
          </Button>
        </span>
    ...
    <div>
      <Grid container spacing={7} >
        ...
        <section className='pt-10 ' >
          ...
          <div className='flex flex-wrap space-y-5 px-20' >
            {same.content?.map((item)=> <HomeSectionCard product={item}/>)}
          </div>
        </section>
